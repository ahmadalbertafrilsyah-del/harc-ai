...
import { 
  LayoutDashboard, Users, MessageSquare, LogOut, FileCheck2, 
  BookOpen, Settings, Clock, ChevronLeft, ChevronRight, UserCircle,
  BarChart4, BrainCircuit, Menu, X, Coins
} from "lucide-react";
      ...
      if (user) {
        const unsubProfil = onSnapshot(doc(db, "users", user.uid), (docSnap) => {
          if (docSnap.exists()) {
            ...
            setProfil({
              nama: data.nama || "Guru Baru",
              spesialisasi: data.spesialisasi || "Pendidik",
  ...
  return (
    <div className={`min-h-screen bg-[#f8fafc] flex ${latoFont.className}`}>
      ...
      <aside className={`hidden md:flex flex-col bg-[#1e3a8a] text-blue-100 transition-all duration-300 z-50 border-r border-blue-900 relative ${isSidebarCollapsed ? "w-[80px]" : "w-[240px]"}`}>
        ...
        <div className="h-16 flex items-center justify-center px-4 border-b border-blue-900/60 bg-[#172554] shrink-0">
          <Link href="/dashboard/guru/beranda" className="flex items-center gap-2.5 overflow-hidden w-full justify-center">
            ...
            {!isSidebarCollapsed && (
              <span className={`text-lg font-bold text-white tracking-wide truncate ${teachersFont.className}`}>Portal Guru</span>
            )}
          </Link>
        </div>
        ...
        <nav className="flex-1 py-6 flex flex-col gap-1 overflow-y-auto overflow-x-hidden custom-scrollbar">
          {!isSidebarCollapsed && (
            <div className="text-[10px] font-bold text-blue-400/80 uppercase tracking-widest mb-2 px-6 truncate">Alat Pendidik</div>
          )}
          {menuItems.map((item) => {
            const isActive = pathname.startsWith(item.path);
            return (
              <Link key={item.name} href={item.path} title={isSidebarCollapsed ? item.name : ""}>
                <div className={`flex items-center px-6 py-3 transition-all text-sm font-medium border-l-[3px] ${isActive ? "bg-blue-900/50 text-white border-blue-400" : "border-transparent text-blue-300 hover:bg-blue-900/30 hover:text-blue-100"} ${isSidebarCollapsed ? "justify-center px-0" : "gap-3"}`}>
                  <item.icon size={20} className={`shrink-0 ${isActive ? "text-blue-400" : "text-blue-400/60"}`} />
        ...
        <div className="p-4 border-t border-blue-900/60 bg-[#172554]">
          <button onClick={handleLogout} title={isSidebarCollapsed ? "Keluar Sistem" : ""} className={`flex items-center text-blue-300 hover:bg-rose-500/20 hover:text-rose-300 w-full rounded-lg transition-all text-sm font-medium ${isSidebarCollapsed ? "justify-center p-2.5" : "gap-3 px-4 py-2.5"}`}>
        ...
        <header className="h-16 bg-white border-b border-slate-200/80 flex items-center justify-between px-4 md:px-6 shrink-0 z-30">
          ...
          <div className="hidden md:flex items-center justify-end gap-5">
            ...
            <div className="flex items-center gap-3 cursor-pointer hover:bg-slate-50 p-1.5 pr-3 rounded-xl transition-colors">
              ...
              <div className="w-9 h-9 bg-blue-700 text-white rounded-lg flex items-center justify-center text-xs font-bold shadow-sm">
          ...
          return (
            <Link key={item.name} href={item.path} className="flex-1 flex flex-col justify-center items-center h-full group">
              <div className={`p-1.5 rounded-xl transition-all mb-1 ${isActive ? "bg-blue-100 text-blue-700" : "text-slate-400 group-hover:text-blue-500"}`}>
                <item.icon size={22} className={isActive ? "fill-blue-100/50" : ""} />
              </div>
              <span className={`text-[10px] font-bold transition-all ${isActive ? "text-blue-700" : "text-slate-500"} truncate px-1`}>
                {item.name}
              </span>
            </Link>
          );
